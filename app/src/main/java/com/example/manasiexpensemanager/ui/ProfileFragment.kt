...
//        saveButton.isEnabled = !(newName=="" || newMonthlyBudget=="")
        ...
        saveButton.setOnClickListener {
            var newName = user_profile_name.editText?.text.toString()
            var newMonthlyBudget = user_month_budget.editText?.text.toString()
            var newIncome = user_month_income.editText?.text.toString()
    ...
    private fun setValues(name: String, monthlyBudget: Float, income: Float) {
        user_profile_name.editText?.setText(name)
        user_month_budget.editText?.setText(monthlyBudget.toString())
        if(income!=0f)
            user_month_income.editText?.setText(income.toString())
    }
    ...
    private fun enableFields() {
        user_profile_name.isEnabled = true
        user_month_budget.isEnabled = true
        user_month_income.isEnabled = true
        saveButton.isEnabled = true
    }

    private fun disableFields() {
        user_profile_name.isEnabled = false
        user_month_budget.isEnabled = false
        user_month_income.isEnabled = false
